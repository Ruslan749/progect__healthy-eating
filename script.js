...
const tabs = document.querySelectorAll('.tabheader__item'), // табы на которые будем кликать (каждый таб по отдельности)
tabsContent = document.querySelectorAll ('.tabcontent'), // вессь контент который меняеться при нажатии табов
...
item.classList.remove('tabheader__item_active'); // убераем класс активности
});
}
...
const deadline = '2021-05-11'; // отправная точка 
...
const modalTimerId = setTimeout(openModal, 3000); //  запускаем модельное окно (используем функцию (opeModal) через 3 секунды
...
openModal(); // запустить модельное окно
...
openModal(); // выполняеться функция открытия модельного окна
